...
public class GDSPNRTTYMsgCreator {
	...
	public GDSPNRTTYMsgCreator() {
		initiateDBService();
	}
	public List<List> getPNRCreateTTYMessage() {
		List<List> fullTTYMessage;
		List<String> ttyFragment;
		...
		//below boolean will be set if the Initial TTY with "NN" is processed
		boolean initialTTYMsgProcessed = false;
		
		try {	
			FileUtility flUtility = new FileUtility();
			fullTTYMessage = new ArrayList<List>();
			ttyFragment = new ArrayList<String>();
			getInitialTTYString("NN");
			cntRec=this.ttyMessageList.size();
			ttyFragment = this.ttyMessageList;
			
			//CHD SSR
			if(assignTTYInputs()==true) {
				//Addition for first executing TTY without SSRs
				
				if(this.childSSRList!=null && this.childSSRList.size()>0) {
					//updated 23Feb
					if(!initialTTYMsgProcessed) {
						fullTTYMessage.add(ttyFragment);
						initialTTYMsgProcessed = true;
						getInitialTTYString("HK");
						ttyFragment = this.ttyMessageList;
					}
					cntRec=ttyFragment.size();
					...
					while (lIter.hasNext()) {
						if (cntRec%24==1) {
							...
							while(iter1.hasNext()) {
								ttyFragment.add(iter1.next());
								cntRec++;
							}
						}						
						ttyFragment.add(lIter.next());	
						cntRec++;				
					}
				}			
					...
					while (lIter.hasNext()) {
						...
						if(tempString.length()>63) {
							if(cntRec%24 < 23 && cntRec%24 > 1) {
								ttyFragment.add(tempString.substring(0, 63));
								cntRec++;
								ttyFragment.add("SSRINFTCM///"+tempString.substring(63, tempString.length()));
								cntRec++;
							}else {
								lIter.previous();
								cntRec++;
				...
				fullTTYMessage.add(ttyFragment);
				flUtility.writeTTYToFile("GDS PNR Creation - TTY Message(s) ", fullTTYMessage);
			}
		...
		try {
			...
			if(assignTTYInputs()==true) {
				...
				if(this.gdsPnrNumber!=null && this.gdsPnrNumber.length()>0) {
					//Nothing to do here
				}else if(CommonProperties.gdsPNR!=null && CommonProperties.gdsPNR.length()>0){
					this.gdsPnrNumber = CommonProperties.gdsPNR;
				}else {
					this.gdsPnrNumber = this.getGDSPNR();
					CommonProperties.gdsPNR=this.gdsPnrNumber;
				}
	...
	public List<List> getSSRTKNETTYMessage() throws IOException {
		List<List> fullTTYMessage;
		List<String> ttyFragment;
		Iterator<String> iter1;
		ListIterator<String> lIter, ssrIter;
		String tempString, surName, givName;
		List<String> ssrTKNEMsgList;
		int cntRec=0, cntSSREntry=0;
		HashMap<String, String> individualPaxDetails = new HashMap<String, String>();
		...
		try {	
			...
			if(assignTTYInputs()==true) {
				...
				if(ssrTKNEMsgs!=null && !ssrTKNEMsgs.isEmpty()) {
					//Adding non infant SSR TKNE
					ssrTKNEMsgList = ssrTKNEMsgs.get("NINF");
					if(ssrTKNEMsgList!=null && ssrTKNEMsgList.size()>0) {
						lIter=ssrTKNEMsgList.listIterator();
						while (lIter.hasNext()) {
							if (cntRec%24==1) {
								fullTTYMessage.add(ttyFragment);
								ttyFragment = new ArrayList<String>();
					...
					//Infant SSR
					ssrTKNEMsgList = ssrTKNEMsgs.get("INF");
					if(ssrTKNEMsgList!=null && ssrTKNEMsgList.size()>0) {
						lIter=ssrTKNEMsgList.listIterator();
						while (lIter.hasNext()) {
							if (cntRec%24==1) {
			...
			flUtility.writeTTYToFile("SSR TKNE Addition - TTY Message(s)", fullTTYMessage);
			return fullTTYMessage;
		}catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public HashMap<String, List<String>> getSSRTKNEMessages(){
		HashMap<String, List<String>> ssrTkneMsgs = new HashMap();
		Iterator iter, strIter;
		HashMap<String, String> individualPaxDetails = new HashMap<String, String> ();
		int i=1;
		String surName, givName, strTKNEMsg, paxType;
		List<String> ssrTKNE, adtSSRTKNEMsgs, infSSRTKNEMsgs;
		try {
			if(CommonProperties.ssrTKNEMsgs!=null && !CommonProperties.ssrTKNEMsgs.isEmpty()) {
				adtSSRTKNEMsgs = new ArrayList<String>();
				infSSRTKNEMsgs = new ArrayList<String>();
				while (i<=this.getPaxMap().size()) {
					individualPaxDetails = this.getPaxMap().get(Integer.valueOf(i)); 
					...
					for(Map.Entry<String, List<String>> mEntry: CommonProperties.ssrTKNEMsgs.entrySet()) {
						ssrTKNE = (ArrayList<String>)mEntry.getValue();
						strIter = ssrTKNE.iterator();
						while (strIter.hasNext()) {
							strTKNEMsg = strIter.next().toString();
							if(strTKNEMsg.contains(surName) && strTKNEMsg.contains(givName)) {
								if(mEntry.getKey().startsWith("INF")) {
									infSSRTKNEMsgs.add(strTKNEMsg);
								}else {
									adtSSRTKNEMsgs.add(strTKNEMsg);
								}
							}
						}						
					}					
				...
				ssrTkneMsgs.put("NINF", adtSSRTKNEMsgs);
				ssrTkneMsgs.put("INF", infSSRTKNEMsgs);
			}
		}catch(Exception e) {
			e.printStackTrace();
		}
